...
async function getData(req) {
	...
	var data = (fetch(url + req)).then((res) => res.json());
...
window.addEventListener('load', function () {
	...
	async function main() {
		...
		// This stuff runs immediately when the page loads
		...
		var dialogsObserver = new MutationObserver(mutations => {
			...
			function addTrait(value, description, color, id) {
				...
				if (document.getElementsByClassName("tetra_tag_holder").length == 1 || document.getElementsByClassName("tetra_tag_holder")[document.getElementsByClassName("tetra_tag_holder").length - 1].getElementsByClassName("tetra_tag_gametime").length > 2) {
